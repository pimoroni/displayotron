...
class HelloWorld(MenuOption):
  ...
  '''
  A few helpers for getitng the icons couldn't hurt either!
  '''
  def get_current_icon(self):
    return self.icons[ self.selected_option ]

  def get_next_icon(self):
    return self.icons[ (self.selected_option + 1) % len(self.options) ]
