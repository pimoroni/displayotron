...
import time, urllib, json, threading, subprocess
...
class Deluge(MenuOption):
  ...
  def redraw(self, menu):
    ...
    if self.millis() - self.last_event >= self.auto_cycle_timeout and len(self.items):
      self.selected_item = ((self.millis() - self.last_event - self.auto_cycle_timeout) / self.auto_cycle_speed) % len(self.items)
    ...
    if len(self.items):
      ...
      menu.write_option(row=0,icon=(chr(1) if item['State'] == 'Paused' else chr(0)),text=item['Name'],scroll=True)
